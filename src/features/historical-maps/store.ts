...
interface HistoricalMapsState {
  /** Whether the historical map time-travel layer is shown on the map. */
  visible: boolean
  /** Layer opacity, 0 (fully transparent) to 1 (opaque background). */
  opacity: number
  /** Selected WMTS Time dimension value: a map edition year from swisstopo. */
  time: string
  /** Map editions reported by swisstopo's WMTS capabilities, newest first. */
...
export const useHistoricalMapsStore = create<HistoricalMapsState>((set) => ({
  visible: false,
  opacity: 1,
  time: 'current',
  ...
  setAvailableTimes: (availableTimes) =>
    set((state) => ({
      availableTimes,
      // Default the comparison edition to the oldest available one distinct
      // from `time`, so enabling compare mode immediately shows a
      // meaningful then-vs-now swipe instead of two identical editions.
